...
const SOUTH_MOUNTAIN_COUNT = 12;
const WEST_MOUNTAIN_COUNT = 11;
...
function buildTerrain(data) {
  ...
  for (
    ...
    if (n < 0.38) {
       ...
       * Valley / exposed soil.
       *
       * Warmer earth tones so the ground
       * does not merge with the radar green.
       */
      const t =
        n / 0.38;
       ...
       * Vegetated terrain.
       *
       * Muted olive rather than saturated
       * radar-green.
       */
      const t =
        (n - 0.38) /
      ...
      /*
       * Exposed high terrain / rock.
       *
       * Desaturated gray-brown rock.
       */
      const t =
        (n - 0.72) /
        0.28;
...
function Forest({
  ...
  const trees =
    useMemo(() => {
      ...
      /*
       * SOUTH MOUNTAIN / CLIFF FORMATIONS
       *
       * A wider, broken ridge along the southern
       * horizon. These stay outside the central
       * radar installation.
       */
      for (let i = 0; i < SOUTH_MOUNTAIN_COUNT; i++) {
        const x =
          -9.0 +
          i * 1.65 +
          (random() - 0.5) * 0.75;
      ...
      /*
       * WEST MOUNTAIN / CLIFF FORMATIONS
       *
       * A staggered western ridge with gaps,
       * rather than a solid artificial wall.
       */
      for (let i = 0; i < WEST_MOUNTAIN_COUNT; i++) {
        const x =
          -7.2 +
          (random() - 0.5) * 2.0;
